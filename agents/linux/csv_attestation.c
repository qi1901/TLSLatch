...
static int verify_impl(struct tcpra_csv_context *context, const uint8_t bytes[RA_REPORT_SIZE],
...
int tcpra_csv_verify_report(struct tcpra_csv_context *context, const uint8_t report[RA_REPORT_SIZE],
    ...
    int ok = verify_impl(context, report, expected);
    prof_end(context->reporter ? "server_self_verify" : "evidence_verify", 0, ok, t);
    return ok;
}
...
int tcpra_csv_generate_report(struct tcpra_csv_context *context, const uint8_t key[RA_KEY_SIZE],
    ...
    pthread_mutex_lock(&context->library_mutex);
    struct prof_stamp api_start = prof_start();
    int rc = context->get_report((void *)key, RA_KEY_SIZE, report, RA_REPORT_SIZE);
    prof_end("csv_get_report_api", 0, rc == 0, api_start);
    pthread_mutex_unlock(&context->library_mutex);
    if (rc != 0) {
